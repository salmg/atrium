...
def bin_to_hex(b):
    """Convert a single byte (int or 1-byte bytes) to a 2-char uppercase hex string."""
    if isinstance(b, int):
        b = bytes([b])
    return binascii.b2a_hex(b).upper().decode('ascii')
...
class APDUCommand(object):
    ...
    def __init__(self, msg, command_desc):
        ...
        if len(msg) == 5:  # LE
            ...
            indexes['lc'] = 4
            indexes['data'] = 5
            lc_byte = msg[indexes['lc']]
            # bytes[i] gives int in Python 3, str[i] gives char in Python 2
            data_size = lc_byte if isinstance(lc_byte, int) else ord(lc_byte)
            data_end = 5 + data_size
            if len(msg) > data_end:  # LC | DATA | LE
                indexes['le'] = data_end

        self.indexes = indexes
        self.parts = {v: k for k, v in indexes.items()}
...
class APDUPrinter(object):
    ...
    def hexdump(self, cmd, indent=0, short=False, linelen=16, offset=0):
        """Generate a coloured hex dump of cmd.msg."""

        def hexable(data, addr):
            elems = cmd.color([bin_to_hex(b) for b in data], addr)
            if not short:
                elems += ["  "] * (linelen - len(elems))
            return " ".join(elems)

        def printable(data, addr):
            chars = [chr(b) if chr(b) in _myprintable else "." for b in data]
            return "".join(cmd.color(chars, addr))

        # Normalise to bytes so indexing always gives ints
        data = cmd.msg
